...
import com.example.aiweb.repository.entity.Reply;
...
public class ReplyServiceImpl implements ReplyService {
    ...
    @Override public List<ReplyDto> findByPostId(Long postId) {
        return repo.findAll().stream()
                .filter(r -> r.getPostId().equals(postId))
                .map(r -> new ReplyDto(r.getId(), r.getPostId(), r.getCommenter(), r.getText(), r.getCommentedAt()))
                .collect(Collectors.toList()); }
    @Override @Transactional public ReplyDto create(ReplyDto dto) {
        Reply r = new Reply(); r.setPostId(dto.getPostId()); r.setCommenter(dto.getCommenter()); r.setText(dto.getText());
